...
try:
    ...
    f = float(f)
    print('Logarithmic growth rate f = %g' % f)
except KeyError:
    f = 0.
...
b_tot = b10*(1+0*mu_mesh)#+(betaf*fnl)*inv_M_mesh+f*mu_mesh**2.
...
factor = beta*5/3
Pmtheta = Plin*factor
Pspecies = {"mm": Plin}#, 'mt': Pmtheta, 'tm': Pmtheta} #mt is m-theta

est = es.Estimator(minkhrec, maxkhrec, K, mu, Ptot, Plin, Pspecies, Pnlinsign, b_tot, nhalo)
...
est.generateNs(K_of_interest, mu_of_interest, minkhrec, maxkhrec, specific_combs,
...
get_additional_values = False
for a, b in listKeys:
    if get_additional_values:
        dic['N'+a+b] = est.getN(a, b, get_additional_values = get_additional_values)
        #values = dict(values)
        #values = dict(values.item())
        #values["Ks"] = est.Krange
        #np.save(direc+data_dir+'values'+a+b+'.npy', values)
    else:
        dic['N'+a+b] = est.getN(a, b)
    dic['N'+b+a] = dic['N'+a+b]
    np.savetxt(direc+data_dir+'N'+a+b+'.txt',np.array((est.Krange,dic['N'+a+b])).T)
    print("Saved", a, b)

print("DONE Noise calculation!")
