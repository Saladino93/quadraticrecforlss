...
class Estimator(object):
    ...
    def double_integrate_for_shot(self, a, K, mu_sign, minq, maxq, mu_sign_prime,
        ...
        if vegas_mode:
            ...
            function = self._double_outer_integral_vegas_for_g(function, self.f, K,
                                        subtract_qtot, mu_sign, mu_sign_prime, a)
            integ = vegas.Integrator([[0, np.pi], [0, np.pi], [0, 2*np.pi], [0, 2*np.pi],
                                    [minq, maxq], [minq, maxq]], nhcube_batch = 1000)
            result = integ(function, nitn = nitn, neval = neval)
            integral = result.mean
    ...
    def get_trispectrum_shot_noise(self, a, minq = None, maxq = None, vegas_mode = True, verbose = True):
        ...
        self.trispectrum_shot = self._get_tris_shot_noise(a, self.Krange, self.murange,
                                            minq, maxq, vegas_mode = vegas_mode)
